#!/usr/bin/env python3
"""Validate bundled Google Scholar profile links against Scholar profile titles.

The script is intentionally conservative:

- cached URLs are never fetched again unless --refresh is passed
...
DEFAULT_DATA = APP_ROOT / "data" / "umap-data.js"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", type=Path, default=DEFAULT_DATA, help="Path to a bundled *-data.js file.")
...
def unique_profiles(rows: list[dict[str, Any]]) -> list[ScholarProfile]:
    ...
    for row in rows:
        ...
        if not url or url in seen:
            continue
        seen.add(url)
        profiles.append(ScholarProfile(index=int(row["Index"]), name=row["Full Name"], url=url))
    return profiles
...
def fetch_profile(url: str) -> dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(request, timeout=20) as response:
            body = response.read(350_000).decode("latin1", errors="replace")
            status_code = response.status
    except urllib.error.HTTPError as error:
        return {"status": "http_error", "status_code": error.code, "title": "", "fetched_at": fetched_at}
    except urllib.error.URLError as error:
        return {"status": "url_error", "error": str(error.reason), "title": "", "fetched_at": fetched_at}
    except TimeoutError:
        return {"status": "timeout", "title": "", "fetched_at": fetched_at}

    lowered = body.lower()
    title = extract_title(body)
    if "not a robot" in lowered or "unusual traffic" in lowered or "captcha" in lowered:
        status = "blocked"
    elif not title:
        status = "no_title"
    else:
    ...
    return {"status": status, "status_code": status_code, "title": title, "fetched_at": fetched_at}
